...
# From https://raw.githubusercontent.com/dfyx/rmxp_rts/master/Scripts/Imported/Priority_Queue.rb
...
class PriorityQueue
  ...
  class Node # :nodoc:
    attr_accessor :parent, :child, :left, :right, :key, :value, :priority, :degree, :mark
  ...
  begin
    ...
    def initialize(key, priority, value = nil)
      @key = key; @priority = priority; @value = value; @degree = 0
    end
  end
  ...
  #     q["car"] = 0
  #     q.min #=> ["car", 0]
  def change_priority(key, priority, value = nil)
    return push(key, priority, value) unless @nodes[key]
  ...
  def push(key, priority, value = nil)    
    return change_priority(key, priority, value) if @nodes[key]
    @nodes[key] = node = Node.new(key, priority, value)
  ...
  def each
    @nodes.each do | key, node |
      yield(key, node.priority, node.value)
    end
  end
  ...
  #     q.min          #=> nil
  def min
    [@min.key, @min.priority, @min.value] rescue nil
  end
  ...
  begin
    ...
    @length -= 1
    return [n.key, n.priority, n.value]
  end
